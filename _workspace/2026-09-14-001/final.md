...
<!-- HUMANIZE-SUMMARY
원본/윤문본 글자수: 제공된 작성 브리프 10,264자 / 최종 본문 9,488자
변경률: 7.6% (브리프를 실제 기술 블로그 본문으로 재구성한 작업이라 문자 단위 변경률은 보조 지표로 기록)
탐지 건수: A-2 3→0, A-10 14→0, C-11 10→0, D-1 10→2, H-1 12→2, I-4 3→1, J-2 5→0
자체검증: 6/6 통과 — Java 고유명사·API·코드·수치·내부 링크·SVG 경로를 확인했고, 기술 블로그 장르와 학습 회고 문체를 유지했으며 S1 패턴이 남지 않았다.
등급: A — S1 잔존 0건, S2 핵심 신호 2건 이하, 반복적인 지시문·불릿·권고형 종결을 본문 흐름과 개인 회고로 정리했다.
주요 변경 하이라이트:
1. 배열에서 Collection으로 넘어가는 이유를 크기 변경이 아니라 저장·탐색 책임의 분리로 재구성
2. ArrayList와 LinkedList의 차이를 삽입·삭제 위치를 찾는 비용까지 포함해 설명
3. HashSet의 hashCode() → equals() 흐름과 가변 Key 문제를 하나의 동등성·불변성 맥락으로 연결
4. Raw Type의 런타임 오류를 Generic의 컴파일 시점 타입 검사와 대비
5. ? extends와 ? super를 실제 타입을 어디까지 확정할 수 있는지에 따라 PECS로 정리
-->
