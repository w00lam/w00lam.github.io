...
<!-- HUMANIZE-SUMMARY
초안/윤문본 글자수: 6,648자 / 6,508자
변경률: 22.8% (문자 편집 거리 기준; 초안→최종)
탐지/개선: A-10 6→0, C-11 8→0, E-2 2→0, D-1 0→0, H-1 0→0, J-3(본문) 0→0
자체검증: 6/6 통과 — 의미·기술 사실·고유명사 보존, 변경률 범위, TIL 장르와 문체 유지, S1 잔존 없음, 임의 비유 추가 없음
등급: A — S1 잔존 0건, S2 잔존 0건, 변경률 10~25% 범위.
주요 변경 하이라이트:
1. 생성자·초기화·소멸 Callback의 역할을 생명주기 흐름으로 분리
2. Singleton·Request·Prototype Scope의 공유 범위와 Prototype 자원 정리 책임을 구분
3. Singleton에 Prototype을 주입할 때 인스턴스가 만들어지는 시점을 설명
4. Annotation, Reflection, Component Scan의 관계를 구분해 설명
5. 요청 상태 공유를 Thread Safety와 연결하고 구조도를 추가
-->
