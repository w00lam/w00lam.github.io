...
서비스 간 순환참조를 해결하는 것이었다.
당시에는 순환참조를 끊기 위해 Facade를 도입했다고 생각했다.
하지만 피드백 이후 코드를 다시 돌아보며 책임을 해석해 보니 내가 Facade라고 인식했던 계층은 단순한 조회 조합을 넘어 하나의 비즈니스 흐름을 실행하는 역할까지 맡고 있었다.

처음에는 Facade 패턴을 적용했다고 생각했지만
실제로는 Facade와 Application Service의 성격이 섞인 형태였다.

이 경험으로 두 패턴을 구분하는 기준이 서비스 호출 개수나 메서드 길이가 아니라 그 클래스의 책임이 조합에 있는지 실행에 있는지에 달렸다는 걸 더 분명히 이해하게 됐다.
...
실제로 더 중요한 역할은 여러 도메인 또는 하위 기능을 조율해 클라이언트가 원하는 형태의 결과를 제공하는 것이다.

Facade는 내부의 복잡한 흐름을 감추고 외부에는 단순한 진입점을 제공한다.
...
서로 다른 책임을 가진 기능들을 조합해 클라이언트 친화적인 응답을 만들어낸다는 데 있다.
...
책임의 경계를 충분히 의식하지 못한 채 구조를 먼저 만들었다는 데 있었다고 생각한다.
...
## 4. 기준은 호출 수가 아니라 책임의 무게중심이다

Facade와 Application Service를 나누는 기준은 다음처럼 볼 수 있다.
...
그래서 아래 예시는 현재 코드베이스에 그대로 적용하기 위한 제안이라기보다 책임과 의존 방향을 설명하기 위한 개념적 예시로 이해하는 편이 더 적절하다.
...
인터페이스는 어떻게 구현하는가보다
이 역할이 무엇을 제공하는가를 명확히 드러내는 추상화 수단에 가깝다.

## 6. 이번 리팩토링에서 얻은 기준

이번 리팩토링으로 가장 크게 정리된 기준은 다음 한 문장으로 요약할 수 있다.
...
이 질문의 답이 클래스의 성격을 더 명확하게 드러내 준다.
