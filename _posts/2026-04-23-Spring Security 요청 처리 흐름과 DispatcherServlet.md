...
> 요청이 필터를 거쳐 인증과 인가를 수행하고 DispatcherServlet로 컨트롤러로 전달되는 흐름 정리
...
- 검증이 성공하면, 이 사용자가 누구인지의 인증 정보를 만든다.

JWT 자체가 인증을 “완료”하는 것이 아니라,
...
현재 요청의 사용자가 누구인지 계속 참조해야 하기 때문이다.

SecurityContext는 현재 요청에서 사용할 인증 정보를 담아두는 공간이다.
