...
작은 실험이라면 에이전트가 어느 정도 알아서 판단해도 괜찮다. 하지만 팀 프로젝트에서는 이야기가 달라진다. 요구사항이 명확하지 않으면 에이전트가 빈칸을 임의로 채우고 정책이나 예외 케이스, API 경계가 문서마다 다르게 흩어질 수 있다. 구현은 빠르게 끝난 것처럼 보여도 나중에 "왜 이렇게 만들었지?"를 추적하기 어려워진다.
...
공식 저장소를 살펴보니 `spec-kit`은 Spec-Driven Development를 시작할 수 있게 도와주는 도구킷에 가깝다. README에서는 Spec-Driven Development를 코드보다 명세를 먼저 중심에 두는 개발 방식으로 설명한다. 코드를 바로 작성하는 것이 아니라 요구사항과 계획을 먼저 구조화하고 그 결과를 바탕으로 구현을 진행하는 흐름이다.
...
여기에는 코드 품질 기준, 테스트 기준, 사용자 경험 기준, 성능 기준, 팀의 개발 원칙 같은 내용이 들어간다. 한 번 정해두면 이후 기능 명세, 기술 계획, 작업 분해를 판단하는 기준이 된다.
...
기술 스택, 아키텍처, 데이터 저장 방식, 외부 연동 방식, API 계약 같은 내용이 이 단계에서 정리된다. 중요한 것은 계획이 앞선 명세와 constitution을 기준으로 만들어진다는 점이다.

기술 선택이 단순히 "요즘 많이 쓰는 방식"으로 흘러가는 것이 아니라, 앞에서 정리한 요구사항과 프로젝트 원칙에 맞는지 검토되는 구조다. README에서도 plan 단계가 명세를 읽고 기술적 결정으로 변환하며 데이터 모델이나 API 계약 같은 문서를 생성하는 흐름을 설명한다.
...
이 단계가 있어야 GitHub Issue, PR, 커밋 단위와 연결하기 쉽다. 기능을 한 번에 "구현"하는 것이 아니라, 사용자 스토리나 의존성에 맞춰 작업을 나누고 병렬로 처리할 수 있는 항목을 구분할 수 있기 때문이다.
...
현재 팀 프로젝트에서는 요구사항 문서, MVP 문서, 정책 문서, API 스펙 문서가 서로 다른 내용을 말할 위험이 있다. v1과 v2 API가 생겼을 때 어떤 문서가 최신 기준인지 헷갈리기도 하고 정책 변경 사항을 아카이브하지 않으면 왜 바뀌었는지 추적하기 어려워질 수도 있다.
...
- 구현 전에 요구사항을 구조화한다.
- AI 에이전트가 임의로 판단하는 범위를 줄인다.
- 문서, 이슈, PR, 구현 사이의 추적성이 좋아진다.
- 팀 프로젝트에서 정책과 경계를 맞추기 쉬워진다.
- 기능 단위로 작업을 쪼개기 좋다.

반대로 한계도 있다.

처음에는 문서 작성 단계가 늘어난 것처럼 느껴질 수 있다. 작은 실험 프로젝트나 빠른 프로토타입에서는 과하게 느껴지기도 한다. 기존 기능을 수정하거나 명세를 계속 진화시키는 흐름은 별도 운영 규칙이 필요해 보인다.
