...
JVM(Java Virtual Machine)의 핵심 구성 요소인 Class Loader, Runtime Data Area, Execution Engine, Garbage Collector가 어떻게 상호작용하며 자바 프로그램을 실행하는지 정리한다.
...
## 회고
자바의 구동 원리를 어렴풋이만 알고 있었는데 이번 기회에 내부에서 구체적으로 어떻게 동작하는지 깊이 있게 들여다볼 수 있었다. 특히 단순히 '힙 메모리를 비워주는 것'으로만 알았던 GC가 세대별로 나뉘어 체계적으로 작동하는 과정을 보며 자바라는 언어에 더 큰 흥미를 느끼게 되었다.
