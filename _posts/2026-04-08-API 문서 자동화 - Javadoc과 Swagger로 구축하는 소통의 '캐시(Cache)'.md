...
permalink: /posts/api-doc-javadoc-swagger/
---

**한 줄 요약: 문서화는 더 이상 노동이 아니다. 코드와 동기화된 문서는 팀의 연산 비용을 줄여주는 최적화다.**

---

## 1. Javadoc vs Swagger — 무엇이 다른가?

두 도구 모두 문서화를 돕지만 누구에게 무엇을 보여줄 것인가에 따라 역할이 나뉩니다.
...
> Javadoc을 더 깊이 들여다본 내용이 궁금하다면 아래 글을 참고해보세요.
> [Javadoc - 코드를 제품으로 격상시키는 문서화의 기술](/posts/javadoc-documentation/)

---

## 2. 문서 자동화는 어떻게 이루어지는가?

문서화가 자동으로 이뤄지는 원리는 정적 분석과 런타임 스캔으로 나뉩니다.
...
2. **Swagger의 런타임/빌드 타임 스캔:** Spring Boot 환경에서는 `springdoc-openapi` 라이브러리가 애플리케이션 실행 시 컨트롤러(`@RestController`)와 DTO(`@Schema`)를 스캔합니다.
   * 리플렉션(Reflection)으로 어노테이션에 담긴 메타데이터를 추출하여 **OpenAPI Spec(JSON/YAML)**을 생성하고 이를 Swagger UI가 화면으로 렌더링합니다.
...
1. **Single Source of Truth (단일 진실 공급원):** 코드와 문서가 분리되면, 코드가 바뀔 때 문서는 과거의 유물이 됩니다. 자동화로 코드 자체가 곧 문서가 되게 해서 동기화 오류를 원천 차단합니다.
2. **협업의 마찰 감소 (Communication Caching):** 프론트엔드 개발자가 "이 API 응답 값이 뭐예요?"라고 물을 필요가 없습니다. 이미 Swagger라는 캐시에 최신 정보가 기록돼 있기 때문입니다.
3. **신뢰성 있는 개발 환경:** Swagger UI에서 API를 직접 호출해보며 개발 서버의 상태를 즉각 확인하고 디버깅할 수 있는 샌드박스 환경을 제공합니다.
4. **온보딩 비용 절감:** 새로 합류한 팀원이 Javadoc로 시스템의 도메인 로직과 예외 처리 방식을 코드를 일일이 뜯어보지 않고도 빠르게 파악할 수 있습니다.
...
### 자동화된 기록의 가치
이번에 Javadoc과 Swagger를 정리하며 느낀 점은, 좋은 문서가 곧 동료를 위한 배려라는 것이었습니다. 수동으로 작성한 문서는 시간이 지나면 관리되지 않는 부채가 되지만 자동화된 문서는 코드가 살아 있는 한 코드와 함께 갱신되는 자산이 됩니다.

지난 시간 배운 **Space-Time Trade-off** 관점에서 보면, 어노테이션을 달고 설정을 잡는 짧은 투자는 프로젝트 내내 쏟아질 수백 번의 질문과 오해를 미리 없애는 최적화입니다.

### 다짐
"나중에 문서 정리해야지"라는 말은 실현되기 어렵다는 것을 잘 알고 있습니다. 코드를 짤 때 Javadoc과 Swagger 어노테이션을 다는 것을 '개발 완성'의 기본 기준으로 삼겠습니다. 내가 만든 API가 나 없이도 스스로를 설명할 수 있게 만드는 것 또한 프로 개발자의 자세임을 잊지 않겠습니다.

---

## 6. References

* **Main Post:** [Javadoc - 코드를 제품으로 격상시키는 문서화의 기술](/posts/javadoc-documentation/)
