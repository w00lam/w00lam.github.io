...
이전 포스트들에서 [VM과 컨테이너의 근본적인 차이](/posts/VM과-컨테이너는-무엇이-다를까?)와 [컨테이너가 어떻게 격리되는지](/posts/컨테이너는-어떻게-격리될까?) 살펴보았다. 컨테이너는 호스트 OS의 커널을 공유하며 가볍게 동작한다고 했는데, 그렇다면 우리가 흔히 사용하는 `ubuntu`나 `alpine` 같은 Docker 이미지는 정말 운영체제(OS)일까? 
...
*   **Host kernel 공유:** 컨테이너는 호스트 OS의 커널을 직접 사용하여 시스템 콜을 처리한다. 이는 [VM과 컨테이너는 무엇이 다를까?](/posts/VM과-컨테이너는-무엇이-다를까/)에서 설명했듯이, 컨테이너가 VM보다 가볍고 빠르게 동작하는 핵심 원리이다.
...
*   [이전 포스트: VM과 컨테이너는 무엇이 다를까?](/posts/VM과-컨테이너는-무엇이-다를까/)
*   [이전 포스트: 컨테이너는 어떻게 격리될까?](/posts/컨테이너는-어떻게-격리될까/)
