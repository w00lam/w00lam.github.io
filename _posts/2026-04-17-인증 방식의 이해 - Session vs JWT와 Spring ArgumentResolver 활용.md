...
### JWT 방식 (Stateless)
서버가 상태를 저장하지 않고 필요한 정보를 토큰 자체에 담아 클라이언트에게 주는 방식입니다.
...
## Spring ArgumentResolver로 파라미터 자동 바인딩하기
...
## 오늘의 회고
세션이 '금고형'이라면 JWT는 '신분증형' 인증입니다. 상황에 맞게 트레이드오프를 고르는 것이 중요하다고 느꼈습니다. **ArgumentResolver**를 직접 구현할 수 있게 되니 컨트롤러 코드를 훨씬 깔끔하게, 의도가 드러나게 관리할 수 있어 매력적이었습니다.
