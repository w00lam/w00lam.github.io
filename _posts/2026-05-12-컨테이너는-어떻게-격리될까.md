...
이전 포스트인 [VM과 컨테이너는 무엇이 다를까?](/posts/VM과-컨테이너는-무엇이-다를까/)에서 컨테이너가 VM보다 가볍고 빠르게 동작하는 이유를 구조적 차이에서 살펴보았다. 컨테이너는 호스트 OS의 커널을 공유하면서도 독립적인 실행 환경을 제공하는데, 이때 핵심적인 역할을 하는 것이 바로 **Linux Namespace**와 **Cgroup**이다.
...
결론적으로, 컨테이너는 VM처럼 하드웨어 전체를 가상화하는 것이 아니라, 호스트 OS의 기능을 활용하여 프로세스 수준에서 격리된 실행 환경을 제공하는 것이다. 이는 [VM과 컨테이너는 무엇이 다를까?](/posts/VM과-컨테이너는-무엇이-다를까/)에서 다루었던 "VM은 가상 컴퓨터, 컨테이너는 격리된 프로세스 실행 환경"이라는 정의를 더욱 명확하게 뒷받침한다.
