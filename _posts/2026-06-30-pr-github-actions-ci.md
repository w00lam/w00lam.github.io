...
`deploy` job은 `build` job이 성공한 뒤에 실행된다. 현재 구조에서는 `master`에 반영된 뒤 Jekyll 빌드를 하고 성공하면 GitHub Pages로 배포하는 흐름이다.

다만 현재 workflow에는 `pull_request` 이벤트가 없다. 그래서 PR을 만들었을 때 자동으로 빌드 검증이 실행되지는 않는다. Docker build나 Gradle build/test 같은 백엔드 빌드 명령은 이 레포의 workflow에는 없다.
