...
image:
  path: /assets/images/2026-06-30-pr-github-actions-ci/cover.png
---
...
16:9 기술 블로그 대표 이미지. 깔끔한 소프트웨어 아키텍처 다이어그램 스타일. PR 생성부터 GitHub Actions, 빌드, 테스트, 코드 리뷰, main 머지, 배포까지 이어지는 흐름을 시각화한다. main 머지 이전에 자동 검증이 완료된다는 점을 강조한다. 빌드 또는 테스트 실패 시 배포 단계로 가지 않고 PR 단계의 수정 흐름으로 되돌아가는 분기 화살표를 표현한다. 이미지 안의 텍스트는 전부 한국어로 작성한다. 포함할 텍스트: "PR 생성", "GitHub Actions", "빌드", "테스트", "코드 리뷰", "main 머지", "배포", "실패 시 PR에서 수정". 전체 분위기는 백엔드 개발과 CI/CD 흐름을 설명하는 차분하고 전문적인 기술 블로그 대표 이미지.
