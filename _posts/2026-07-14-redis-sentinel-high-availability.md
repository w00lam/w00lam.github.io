...
- [1. 단일 Redis 구조의 문제: SPOF](#1-단일-redis-구조의-문제-spof)
- [2. 데이터를 복제하는 Primary-Replica](#2-데이터를-복제하는-primary-replica)
- [3. Replica만으로 자동 복구할 수 없는 이유](#3-replica만으로-자동-복구할-수-없는-이유)
- [4. Redis Sentinel이 담당하는 역할](#4-redis-sentinel이-담당하는-역할)
- [5. Sentinel은 어떻게 장애를 판단할까](#5-sentinel은-어떻게-장애를-판단할까)
- [6. Redis Sentinel Failover 전체 흐름](#6-redis-sentinel-failover-전체-흐름)
- [7. 비동기 복제와 데이터 유실 가능성](#7-비동기-복제와-데이터-유실-가능성)
- [8. 캐시와 핵심 원본 데이터는 영향이 다르다](#8-캐시와-핵심-원본-데이터는-영향이-다르다)
- [9. Redisson 분산 락에서도 남는 주의점](#9-redisson-분산-락에서도-남는-주의점)
- [10. Replica 읽기와 Replication Lag](#10-replica-읽기와-replication-lag)
- [11. Sentinel과 Redis Cluster의 차이](#11-sentinel과-redis-cluster의-차이)
- [12. 현재 프로젝트에 적용한다면](#12-현재-프로젝트에-적용한다면)
- [13. 핵심 요약](#13-핵심-요약)
- [14. 학습하며 바뀐 생각](#14-학습하며-바뀐-생각)
- [15. 마무리](#15-마무리)
...
아래 이미지는 모두 **밝은 배경의 기술 블로그용 인프라 다이어그램, 단순한 2D 벡터 스타일, 가로형 16:9 비율, 장식보다 구조 전달 우선, 불필요한 캐릭터와 3D 효과 제외**라는 공통 스타일을 사용한다. 한글이 정확히 생성되지 않으면 레이블을 후편집할 수 있도록 텍스트 영역을 여백이 있는 박스로 분리한다.

### 이미지 1. 단일 Redis와 SPOF

```text
밝은 배경의 기술 블로그용 2D 벡터 인프라 다이어그램. 가로형 16:9.
왼쪽에 Spring Boot API 서버, 중앙에 단일 Redis 노드, 오른쪽에 상품 검색 캐시,
인기 검색어 캐시, 분산 락, 검색 기록 기능을 배치한다. Spring Boot에서 Redis를 거쳐
각 기능으로 이어지는 연결선을 그린다. Redis 노드에 붉은 경고 아이콘과 장애 표시를 넣고,
장애 이후 오른쪽 기능으로 가는 연결선이 끊어진 모습을 명확히 표현한다.
한글 레이블: "단일 장애점", "SPOF", "Redis 장애", "캐시", "분산 락", "검색 기록".
Redis 장애가 서비스 전체가 아니라 Redis 의존 기능에 영향을 준다는 구조가 드러나야 한다.
텍스트는 짧게, 화살표 방향은 분명하게, 캐릭터와 3D 효과는 제외한다.
```

### 이미지 2. Primary-Replica와 Sentinel 구조

```text
밝은 배경의 기술 블로그용 2D 벡터 인프라 다이어그램. 가로형 16:9.
왼쪽에 Spring Boot 애플리케이션, 위쪽에 서로 분리된 Sentinel 3대,
오른쪽 중앙에 Redis Primary 1대와 그 아래 Redis Replica 2대를 배치한다.
Primary에서 두 Replica로 향하는 단방향 복제 화살표를 그리고 "복제"라고 표시한다.
Sentinel 3대에서 Primary와 Replica로 향하는 점선 감시 화살표에 "감시"라고 표시한다.
Spring Boot가 Sentinel에 "현재 Primary 조회" 요청을 보내고 주소 응답을 받은 뒤,
Redis Primary에 직접 연결하는 두 단계 흐름을 표현한다.
Sentinel과 Primary 사이에 데이터 트래픽이 흐르는 것처럼 그리지 말고,
Sentinel은 프록시가 아니라 감시와 Primary 정보 제공 역할임을 시각적으로 구분한다.
한글 레이블: "감시", "복제", "현재 Primary 조회", "데이터 요청은 Primary로".
```

### 이미지 3. Redis Sentinel Failover 흐름

```text
밝은 배경의 기술 블로그용 2D 벡터 단계형 다이어그램. 가로형 16:9.
왼쪽에서 오른쪽으로 7단계 타임라인을 구성한다.
1 Primary 장애: 기존 Redis Primary에 붉은 장애 아이콘.
2 SDOWN: Sentinel 한 대의 주관적 장애 판단.
3 ODOWN: Sentinel 3대 중 다수가 동의하는 다수결 표현.
4 리더 선출: Sentinel 한 대에 리더 배지.
5 Replica 승격: Replica 중 하나가 "새 Primary"로 역할 변경.
6 Replica 재연결: 나머지 Replica의 복제 화살표가 새 Primary를 향하도록 변경.
7 Spring Boot 재접속: 연결선이 기존 Primary에서 새 Primary로 전환.
단계 사이 화살표를 굵고 분명하게 표시한다.
한글 레이블: "Primary 장애", "SDOWN", "ODOWN", "리더 선출",
"Replica 승격", "Replica 재연결", "재접속".
```

### 이미지 4. 비동기 복제로 인한 데이터 유실 가능성

```text
밝은 배경의 기술 블로그용 2D 벡터 시퀀스 다이어그램. 가로형 16:9.
왼쪽부터 클라이언트, Redis Primary, Redis Replica, Sentinel을 배치한다.
클라이언트가 Primary에 쓰기 요청을 보내고 Primary가 "저장 성공" 응답을 반환한다.
Primary에서 Replica로 향하는 "비동기 복제" 화살표는 중간에서 끊겨 있고,
그 직후 Primary에 장애 아이콘이 나타난다. Sentinel은 Replica의 역할만
"새 Primary"로 전환하고, 누락 데이터를 복구하지 않는 모습을 보여 준다.
마지막에 새 Primary 내부에 이전 데이터 버전을 표시한다.
한글 레이블: "비동기 복제", "복제 전 장애", "새 Primary로 승격",
"일부 데이터 유실 가능", "Sentinel은 역할을 전환하고 데이터를 복원하지 않음".
```

### 이미지 5. Primary 읽기와 Replica 읽기 비교

```text
밝은 배경의 기술 블로그용 좌우 비교형 2D 벡터 다이어그램. 가로형 16:9.
왼쪽 영역 제목은 "Primary 읽기", 오른쪽 영역 제목은 "Replica 읽기".
왼쪽 Primary에는 최신 데이터 버전과 주문 상태 "PAID"를 표시하고,
"쓰기 직후 조회", "주문·결제·재고" 레이블을 배치한다.
오른쪽 Replica에는 한 단계 이전 버전과 주문 상태 "PENDING"을 표시하고,
Primary에서 Replica로 향하는 복제 화살표 옆에 작은 지연 시계 아이콘을 넣는다.
오른쪽에는 "약간 오래된 데이터 가능", "통계·랭킹·분석" 레이블을 배치한다.
상단 또는 하단에 "Read-After-Write"와 "Replication Lag" 레이블을 명확히 표시한다.
두 영역의 최신성 차이가 한눈에 보이도록 색상과 버전 번호를 단순하게 구분한다.
```

### 이미지 6. Sentinel과 Redis Cluster 비교

```text
밝은 배경의 기술 블로그용 좌우 비교형 2D 벡터 아키텍처 다이어그램. 가로형 16:9.
왼쪽 제목은 "Redis Sentinel", 오른쪽 제목은 "Redis Cluster".
Sentinel 쪽에는 Primary 1대와 Replica 여러 대를 그리고 모든 노드 안에
동일한 데이터 A, B, C를 표시한다. Primary 장애 시 Replica 승격 화살표를 넣고
"고가용성", "전체 복제", "자동 Failover", "샤딩 없음" 레이블을 사용한다.
Cluster 쪽에는 Primary 여러 대를 그리고 A, B, C 키가 서로 다른 Primary에
분산된 모습을 해시 슬롯 띠와 함께 표현한다. 각 Primary 아래에 전용 Replica를 배치한다.
한글 레이블: "샤딩", "해시 슬롯", "수평 확장", "고가용성".
Sentinel은 동일 데이터 전체 복제, Cluster는 데이터 분산이라는 차이가 즉시 보여야 한다.
```

### 이미지 7. 현재 프로젝트의 단계별 Redis 운영 전략

```text
밝은 배경의 기술 블로그용 3단계 성장 로드맵 2D 벡터 다이어그램. 가로형 16:9.
1단계 "로컬·개발": Spring Boot와 단일 Redis, 레이블 "단순한 개발 환경".
2단계 "초기 운영": 서로 다른 서버 또는 가용 영역에 분산된 Redis Primary,
Replica와 Sentinel 3대, 레이블 "프로세스 장애 대응", "서버 장애 대응".
3단계 "대규모 트래픽": 여러 Redis Primary와 각 Replica, 해시 슬롯 기반 샤딩,
레이블 "수평 확장".
로드맵 아래에 잘못된 예시를 작은 경고 박스로 추가한다. 하나의 EC2 박스 안에
Primary, Replica, Sentinel 컨테이너를 모두 넣고 붉은 경고 아이콘과 함께
"EC2 장애에는 함께 중단"이라고 표시한다.
단계별 성장 방향 화살표가 분명하고, 기술 이름보다 선택 시점이 잘 보이게 구성한다.
```
