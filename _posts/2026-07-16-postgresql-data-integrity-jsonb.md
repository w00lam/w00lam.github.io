...
JSONB와 다양한 인덱스도 선택 이유가 된다. 관계형 컬럼을 중심으로 설계하되 구조가 자주 달라지는 일부 속성만 JSONB로 담을 수 있다. B-tree만으로 풀기 어려운 검색에는 GIN, GiST, BRIN 등의 인덱스 유형을 검토할 수 있다. 데이터베이스 밖에 별도 저장소를 하나 더 도입하기 전에 PostgreSQL 안에서 요구사항을 만족할 여지가 커진다.
...
다만 여기서 “PostgreSQL은 항상 MySQL보다 빠르고 안정적이다”라는 결론은 나오지 않는다. 성능은 쿼리, 인덱스, 데이터 분포, 동시 요청, 설정과 하드웨어에 따라 달라진다. PostgreSQL은 특정 요구사항을 표현할 도구가 풍부하고 그 도구를 적극 활용하는 설계에 잘 맞기 때문에 선택된다.
...
- 충돌 특성에 맞는 격리 수준
- 교착 상태나 직렬화 실패에 대응하는 짧고 제한된 재시도
...
여기서 가장 중요한 점은 JSONB가 관계형 모델을 없애는 기능이 아니라는 사실이다.
...
따라서 “정합성이 중요하면 PostgreSQL, 아니면 MySQL”처럼 나누는 것은 부정확하다. MySQL에서도 제약조건과 트랜잭션을 제대로 설계하면 정합성을 지킬 수 있다. 반대로 PostgreSQL에서도 제약조건을 생략하고 잘못된 동시성 코드를 작성하면 데이터가 깨진다.
...
지금까지는 익숙하다는 이유만으로 MySQL을 선택했다. 이번 학습으로 익숙함 자체가 나쁜 기준은 아니지만, 생산성과 운영 안정성에 영향을 주는 여러 기준 중 하나라는 점을 알게 됐다.
