...
Kafka는 메시지를 비동기로 전달할 수 있게 해준다. 주문이 생성되면 `order-created` 이벤트를 발행하고, 결제가 완료되면 `payment-completed` 이벤트를 발행한다. 그러면 포인트 서비스, 재고 서비스, 알림 서비스 같은 Consumer가 자신에게 필요한 이벤트를 받아 처리할 수 있다.
...
이 구조가 `payment-failed`, `order-created`, `stock-decreased`, `point-earned`, `payment-refunded`, `point-revoked`에도 붙는다고 생각하면 모니터링 대상, 알림 정책, 보관 기간, 재처리 도구가 모두 늘어난다. 토픽을 많이 나누는 것이 항상 좋은 설계는 아니라는 뜻이다.

그래서 토픽은 단순히 이벤트가 존재한다는 이유로 나누기보다 다음 기준으로 나누는 편이 더 현실적이라고 느꼈다.
...
결국 토픽 설계는 이벤트 이름을 나열하는 작업이 아니라, 소비자 관심사와 운영 정책을 나누는 작업에 가깝다.
...
지연 재시도는 모든 이벤트에 붙이는 기본 장식이 아니라, 도메인 정합성에 영향을 주는 이벤트에 우선 적용해야 하는 운영 전략이라고 느꼈다.
...
결국 Kafka 기반 설계에서는 Producer가 올바른 key를 선택하는 것과 Consumer가 현재 상태를 검증하는 것이 함께 필요하다. 파티션 순서는 중요한 힌트지만, 도메인 상태 검증을 대체하지는 못한다.
