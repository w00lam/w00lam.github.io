...
이번 로그는 매니저 등록 데이터의 일부가 아니라 요청 흐름을 추적하기 위한 감사 로그였습니다. 메인 트랜잭션의 성공 여부에 종속되기보다 독립적으로 커밋되는 편이 목적에 더 적합하다고 판단했습니다.
...
AOP는 대상 메서드의 실행 시점을 가로채 공통 관심사를 분리하는 데 사용할 수 있습니다. 이번 구현에서는 여기에 더해 `JoinPoint` 또는 `ProceedingJoinPoint`로 실제 호출 인자도 확인할 수 있다는 점을 활용했습니다.
...
현재 서비스의 Todo에는 담당자를 등록할 수 있습니다. Todo는 개인이 혼자 처리하는 메모라기보다 여러 사용자가 함께 수행할 수 있는 작업 단위로 볼 수 있습니다.
