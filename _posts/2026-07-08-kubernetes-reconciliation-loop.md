...
선언형으로 가려면 상태를 저장할 곳이 필요하다. 그래서 etcd가 필요하다.  
상태를 안전하게 다루려면 모든 요청이 들어오는 단일 관문이 필요하다. 그래서 API Server가 필요하다.  
아직 어느 노드에도 배치되지 않은 Pod가 있다면 적절한 노드를 결정해야 한다. 그래서 Scheduler가 필요하다.  
원하는 상태와 현재 상태의 차이를 계속 메우는 주체가 필요하다. 그래서 Controller Manager가 필요하다.  
각 노드에서 실제로 Pod를 실행하고 상태를 보고해야 한다. 그래서 kubelet이 필요하다.
...
API Server는 모든 요청의 단일 관문이다. `kubectl`로 요청하든, CI/CD 파이프라인이 요청하든, Controller가 상태를 갱신하든 결국 API Server를 통해 클러스터 상태를 다룬다.
...
Service는 Label과 Selector를 기준으로 요청을 보낼 Pod를 찾는다. 예를 들어 Service가 `app=my-spring-app`이라는 Selector를 가지고 있다면, 같은 Label을 가진 Pod들이 대상이 된다.

중요한 점은 Service가 특정 Pod IP에 고정되는 것이 아니라는 점이다. Pod A가 죽고 Pod D가 새로 생기더라도, Pod D가 같은 Label을 가지고 있으면 Service는 새 Pod를 대상으로 삼을 수 있다.
...
GitOps도 같은 맥락에서 이해할 수 있다. Git에 선언된 상태와 클러스터의 현재 상태를 비교하고, 차이가 있으면 클러스터를 Git의 상태에 맞춘다. 결국 여기에도 Desired State, Current State, Reconciliation Loop가 반복된다.
