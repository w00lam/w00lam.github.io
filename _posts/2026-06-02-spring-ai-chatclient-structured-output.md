...
이 글에서는 이 흐름 중 **Structured Output**과 **Tool Calling**을 Spring AI 프레임워크로 어떻게 구현하고 활용할 수 있는지에 초점을 맞춥니다.
...
**Spring AI**는 이러한 문제를 해결하고 AI 모델 호출을 Spring Boot 애플리케이션 내에서 자연스럽게 다룰 수 있도록 돕는 프레임워크입니다. Spring AI를 사용하면 AI 호출도 하나의 외부 의존성으로 간주해 Spring이 제공하는 기능(Bean, 설정, 서비스 계층)으로 체계적으로 관리할 수 있습니다.
...
>  **주의**: 이론 수업에서는 API 형태를 이해하는 것이 목적입니다. 실제 프로젝트에서는 사용하는 Spring AI 버전과 모델 프로바이더에 따라 설정과 일부 옵션이 달라질 수 있습니다.
...
서비스에서 LLM을 활용할 때는 "좋은 말"로 된 답변보다 "다음 코드가 처리 가능한 응답"이 훨씬 중요합니다. LLM의 응답이 애플리케이션의 후속 로직에서 쉽게 파싱되고 활용될 수 있도록 출력 형식을 명확히 지정해야 합니다.
...
`OutputConverter`는 이 JSON 문자열을 `ReceiptExtraction`과 같은 Java 객체로 자동으로 매핑해줍니다. 이 과정에서 개발자는 복잡한 파싱 로직 없이 LLM의 응답을 비즈니스 로직에 바로 활용할 수 있습니다.

## 7. Tool Calling — LLM이 외부 도구를 사용하는 방법

LLM은 방대한 지식을 갖췄지만 실시간 정보나 특정 도메인에 특화된 기능(예: 날씨 정보 조회, 주식 가격 확인, 데이터베이스 쿼리)에는 접근하지 못합니다. **Tool Calling**은 LLM이 이러한 외부 도구들을 필요에 따라 호출하여 자신의 능력을 확장할 수 있도록 하는 메커니즘입니다.

### Tool Calling의 필요성

* **실시간 정보 접근**: LLM의 학습 데이터는 특정 시점까지의 정보만을 포함합니다. Tool Calling로 최신 정보를 조회할 수 있습니다.
...
Spring AI는 `ChatClient`로 Tool Calling을 쉽게 통합하도록 지원합니다. 개발자는 사용할 도구를 정의하고 `ChatClient`에 등록하기만 하면 됩니다. LLM은 대화의 맥락을 파악하여 적절한 시점에 필요한 도구를 호출할지 여부를 결정하고 필요한 인자를 추출하여 전달합니다.
...
**Agentic Application**은 LLM이 질문에 답하는 데서 그치지 않고 스스로 목표를 세우고 계획을 짜고 필요한 도구로 행동하며 그 결과를 평가해 다음 행동을 정하는 자율적인 애플리케이션을 말합니다. 소프트웨어 에이전트가 사람처럼 문제를 풀어 가는 방식에 가깝습니다.

### Agentic Application의 특징

* **자율성**: LLM이 외부의 명시적인 지시 없이 스스로 판단하고 행동합니다.
* **계획 수립**: 복잡한 작업을 작은 단계로 분해하고 실행 계획을 세웁니다.
* **도구 사용**: Tool Calling로 외부 시스템과 상호작용하며 정보를 얻거나 작업을 수행합니다.
...
Spring AI는 LLM을 활용한 AI 앱 개발을 Spring Boot 생태계 안에서 자연스럽고 효율적으로 다루도록 돕는 도구입니다. LLM API를 직접 호출할 때의 한계를 넘어 `ChatClient`로 고수준 추상화를 제공하고 `Structured Output`으로 데이터 중심의 응답을 처리하며 `Tool Calling`으로 LLM의 능력을 확장하는 등 다양한 기능을 지원합니다.

이번 학습에서 AI 앱 개발이 단순히 LLM을 호출하는 데서 그치지 않는다는 점을 확인했습니다. 결국 **어떻게 LLM과 애플리케이션의 비즈니스 로직을 유기적으로 연결하고 LLM의 응답을 서비스에 적합한 형태로 가공하며 LLM의 한계를 외부 도구로 보완할 것인가**에 깊은 고민이 필요합니다. 특히 `Structured Output`과 `Tool Calling`은 LLM을 실제 서비스에 통합할 때 빠질 수 없는 요소이고 이 둘을 잘 활용하면 더 견고하고 지능적인 AI 앱을 만들 수 있습니다.

### Learned (배운 점)

* **LLM API 직접 호출의 한계**: 서비스 규모가 커질수록 프롬프트 관리, 모델 교체, 응답 처리, 고급 기능 통합 등에서 많은 어려움이 발생한다.
* **Spring AI의 역할**: Spring Boot 개발자에게 익숙한 방식으로 LLM 호출을 통합하고 Bean 관리·설정·서비스 계층 분리로 체계적인 AI 앱 개발을 가능하게 한다.
* **`ChatClient`의 중요성**: LLM과의 대화를 위한 고수준 API로, 개발자가 주로 사용하는 인터페이스이다.
* **Prompt 설계의 중요성**: System Message와 User Message로 모델의 역할, 맥락, 제약, 출력 형식을 명확히 정의하는 것이 고품질 응답을 얻는 핵심이다.
...
* **Agentic Application의 비전**: `Tool Calling`로 LLM이 스스로 목표를 세우고 계획을 짜고 행동하며 결과를 평가하는 자율적인 AI 앱 개발의 가능성을 엿볼 수 있었다.
