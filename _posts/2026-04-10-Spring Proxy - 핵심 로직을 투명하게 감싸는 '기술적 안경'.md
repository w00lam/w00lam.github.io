...
> 책에 직접 돋보기를 붙이거나 글씨를 크게 해서 다시 인쇄(코드 수정)할 필요가 없습니다. 그저 **안경(프록시)을 쓰기만 하면** 원본은 그대로인데 내가 보는 **경험(기능)** 만 업그레이드됩니다.
...
* **문제 (Proxy Bypass):** 머릿속에서 일어나는 상상은 안경 렌즈를 통과하지 않습니다. 빛이 굴절되어 시야를 교정하는 물리적 상호작용이 생략됩니다.
* **결과:** `Spring`의 `@Transactional`이나 `@Async` 같은 기능은 AOP 프록시라는 '렌즈'를 통과할 때만 가로채기로 부가 기능이 활성화됩니다.

하지만 클래스 내부에서 this.methodB()와 같이 자신의 다른 메서드를 직접 호출하면 호출 주체가 프록시가 아닌 실제 객체(Target) 본인이 됩니다.
이때 호출 흐름이 프록시를 거치지 않고 실제 객체 내부에서만 순환하는 프록시 바이패스가 발생합니다. 이 경우 @Transactional 어노테이션이 선언되어 있어도 트랜잭션 매니저 같은 인프라 로직이 개입할 수 없어 아무런 기능도 작동하지 않습니다.
...
내가 중요하게 생각하는 **API 문서화(Swagger)**나 **JPA Auditing**도 사실 이런 '투명한 안경'들이 뒤에서 열심히 일해준 결과입니다. "원본의 순수성은 지키되 기능은 확장한다"는 이 철학이 백엔드 설계의 핵심임을 다시금 느낍니다.

### 다짐

이번 학습으로 단순히 @Transactional 어노테이션을 붙이는 것을 넘어 그 이면에서 프록시라는 대리인이 어떻게 인프라 로직을 주입하는지 그 실체를 명확히 이해했습니다.
