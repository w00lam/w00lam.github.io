...
* 그 결과 배포 파이프라인의 `deploy` 단계는 단순한 '명령어 실행' 단계가 아니라, **새 버전이 운영 트래픽을 감당해도 되는지 검증하는 마지막 확인 관문**이어야 한다.
...
롤링 배포를 제대로 활용하려면 **하위 호환성 설계(Backward Compatibility)**, **점진적인 DB 스키마 마이그레이션(Expand/Contract 패턴)**, 그리고 **배포 단계별 헬스체크 기준 설정**과 **실패 시 중단 조건**을 미리 정밀하게 맞춰 두어야 합니다.
...
3. Green 환경에 배포가 완료되면, 운영 트래픽이 유입되기 전에 내부 헬스체크 및 별도 테스트망으로 신버전의 정상 동작 여부를 철저하게 사전 검증합니다.
...
* **한계**: 완전히 동일한 리소스 구성을 두 세트 유지해야 하므로 인프라 구축 및 유지 비용이 크게 증가합니다. 로드밸런서, 타겟 그룹 제어, 외부 라우팅 설정 등 구성 복잡도가 올라갑니다.
...
> **그 결과 CI/CD 설계에서는 "어떤 파이프라인을 실행할 것인가"만큼 "어떤 타이밍과 조건에서 안전하게 트리거할 것인가"도 핵심 고려 대상입니다.**
