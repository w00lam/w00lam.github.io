...
스프링은 어노테이션으로 객체의 생명주기를 관리합니다.

* **`@Component`의 계보:** `@Controller`, `@Service`, `@Repository`는 모두 내부에 `@Component`를 선언하고 있습니다. 덕분에 스프링의 **ComponentScan**이 이들을 빈으로 인식합니다.
...
* **발생 시각:** 사건이 정확히 언제 일어났는지
* **로그 레벨:** 단순한 안내(INFO)인지 즉시 수정해야 할 에러(ERROR)인지
* **쓰레드 이름:** 어떤 쓰레드가 이 작업을 수행 중인지 (멀티 쓰레드 환경에서 디버깅의 핵심)
* **클래스명:** 어느 파일 어느 라인에서 로그가 찍혔는지

### 2. 로그 레벨 관리와 유연성
상황에 따라 출력 범위를 조절할 수 있습니다.
* **DEBUG/TRACE:** 로컬 개발 환경에서 상세한 흐름을 파악할 때 사용.
* **INFO/WARN/ERROR:** 실제 서버 운영 환경에서 시스템 상태를 모니터링할 때 사용.
* 서버를 재시작하지 않고도 설정에서 특정 패키지의 로그 레벨만 바꿔 문제를 추적할 수 있습니다.
