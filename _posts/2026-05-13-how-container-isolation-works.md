...
컨테이너는 VM처럼 하드웨어 전체를 가상화하지 않는다. 대신 호스트 OS의 기능을 활용해 프로세스 수준에서 격리된 실행 환경을 제공한다. 이는 [VM과 컨테이너는 무엇이 다를까?](/posts/vm-container-diff/)에서 다룬 "VM은 가상 컴퓨터, 컨테이너는 격리된 프로세스 실행 환경"이라는 정의를 뒷받침한다.
