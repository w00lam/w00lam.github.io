...
RAG는 모델 내부 매개변수(Weight)를 바꾸는 학습(Training) 기술이 아니라, **백엔드 검색 파이프라인으로 가장 적절한 외부 문서를 찾아 프롬프트(Prompt)라는 제한된 공간에 문맥(Context)으로 주입해 주는 시스템 구조**입니다.
...
3. **환각(Hallucination) 현상**: LLM은 모르는 내용에는 "모른다"고 답하기보다, 확률적으로 그럴듯한 거짓 문장을 생성하는 경향이 있습니다.
...
* **질문 임베딩 생성**: 사용자의 자연어 질문을 동일한 임베딩 모델로 벡터로 변환합니다.
...
* **LLM 답변 생성 & 출처 제공**: LLM은 주입된 문맥만으로 답변을 작성하고 사용된 청크의 원문 링크나 메타데이터를 함께 출력합니다.
...
> ** 개발 기록(TIL)에 적합한 전략**:
> 개발 블로그나 TIL 문서는 기본적으로 Markdown 형식으로 작성되며 헤더(`## 1. 문제 상황`, `## 2. 해결 방법`) 구조가 매우 명확합니다. 단순 고정 길이 분할보다 **Markdown 헤더와 문단을 기준으로 분할하고 상위 헤더 제목을 청크에 포함시키는 구조적 Chunking**이 가장 우수한 검색 품질을 보여줍니다.
...
| **출처 제시** | 메타데이터로 **원문 출처 제시 용이** | 모델 내부 지식이므로 **출처 제시 불가능** |
...
이처럼 RAG 서비스로 머릿속에 파편화되어 있던 과거의 기술적 고민과 회고를 **단 한 줄의 자연어 질문으로 빠르게 소환**할 수 있습니다. (나아가 이러한 기록 통합 관점은 [온톨로지 기반 세컨드 브레인 구축](/posts/ontology-ai-second-brain/)과도 긴밀하게 이어집니다.)
...
* Chunking, Metadata, Hybrid Search, Reranking은 검색 품질을 높여주는 핵심 수단이지만 **초기 MVP에서는 단순한 Vector Search 구조로 빠르게 시작한 뒤 실제 검색 실패 사례 데이터로 단계별 확장을 진행하는 것이 백엔드 엔지니어링 관점에서 바람직**합니다.
