...
현대 웹 서비스에서 **실시간 통신**은 선택이 아닌 필수가 되었습니다. 채팅 애플리케이션, 실시간 알림, 협업 도구, 온라인 게임 등에서 사용자들은 즉각적인 정보 업데이트를 기대합니다. 하지만 웹의 근간을 이루는 HTTP 프로토콜에는 이런 실시간 요구사항을 충족하기 어려운 구조적 한계가 있습니다. 이 글에서는 HTTP의 한계부터 WebSocket, STOMP, 그리고 마지막 인증 문제까지 실시간 서비스를 파고들면서 정리한 내용을 공유하려 합니다.

## HTTP의 구조적 특징과 한계

HTTP(HyperText Transfer Protocol)는 웹의 가장 기본적인 통신 프로토콜입니다. 가장 큰 특징은 **Request-Response** 모델을 따른다는 점입니다. 클라이언트가 요청을 보내면 서버가 응답하는 방식으로 동작하고 서버가 먼저 클라이언트에게 메시지를 보낼 수는 없습니다. 기본적으로 연결 지향적이지만 요청-응답 후 연결을 끊는 특성 때문에, 애초에 실시간 이벤트 전달을 전제로 설계되지 않았습니다.

> **핵심 정리**: HTTP는 클라이언트의 요청의 서버의 응답으로 동작하며 서버 푸시나 지속적인 연결에는 적합하지 않습니다.

### HTTP의 한계 — 실시간 서비스와의 충돌

이런 HTTP의 특성은 실시간 서비스와 충돌합니다. 서버에서 발생하는 이벤트를 클라이언트에게 즉시 전달하기 어려워, **Polling** 같은 비효율적인 방법을 써야 합니다. 그리고 이 방식은 불필요한 요청 증가와 트래픽 낭비로 이어져 실시간 서비스의 성능을 떨어뜨리는 주된 원인이 됩니다.
...
* **동작 방식**: 클라이언트가 일정 시간 간격으로 서버에 HTTP 요청을 보내고 서버는 현재 데이터를 응답합니다.
...
* **단점**: 여전히 HTTP 기반이므로 연결 유지의 오버헤드가 존재하며 서버가 요청을 계속 붙잡고 있어야 하므로 확장성에 한계가 있습니다.

> **핵심 정리**: Polling과 Long Polling은 HTTP의 한계를 우회하려는 시도였지만 근본적인 실시간 통신 문제를 해결하지 못하고 비효율성을 야기합니다.

## WebSocket 등장 배경 — HTTP의 한계를 넘어서

Polling과 Long Polling의 한계를 극복하고 진정한 실시간 양방향 통신을 위해 **WebSocket**이 등장했습니다. WebSocket은 HTTP와는 다른 프로토콜이지만 초기 연결 설정(핸드셰이크)은 HTTP로 이루어집니다. 핸드셰이크가 성공하면 HTTP 연결은 WebSocket 연결로 업그레이드되고 이후부터는 독립적인 TCP 기반의 양방향 통신 채널이 열립니다.
...
* **이벤트 기반 통신**: 메시지 프레임으로 효율적인 데이터 전송이 가능합니다.
...
WebSocket은 클라이언트와 서버 간의 지속적인 양방향 통신 채널을 제공하여 HTTP의 한계를 극복했습니다. 하지만 순수 WebSocket만으로는 복잡한 메시징 애플리케이션을 개발하기에 부족한 점이 많습니다. 예를 들어 특정 사용자에게 메시지를 보내거나, 특정 주제(채팅방)에 참여한 모든 사용자에게 메시지를 브로드캐스트하는 등의 기능은 직접 구현해야 합니다.

* **메시지 목적지 개념 없음**: 메시지를 어디로 보낼지의 표준화된 방법이 없습니다.
...
WebSocket은 **통신 채널**을 제공하지만 그 위에서 동작하는 **메시징 프로토콜**은 아닙니다. '통신은 해결하지만 메시징은 해결하지 못한다'는 한계가 있는 셈입니다.

> **핵심 정리**: 순수 WebSocket은 저수준 통신 채널일 뿐, 복잡한 메시징 애플리케이션 개발에 필요한 메시지 라우팅, 구독/발행 등의 기능을 직접 제공하지 않습니다.

## STOMP 등장 — 메시징의 표준을 제시하다

순수 WebSocket의 메시징 한계를 해결하기 위해 **STOMP(Simple Text Oriented Messaging Protocol)**가 등장했습니다. STOMP는 WebSocket 위에서 동작하는 메시징 프로토콜로, 메시지 송수신의 표준화된 형식을 제공해 복잡한 메시징 로직을 쉽게 구현하도록 돕습니다.
...
Spring 애플리케이션에서 인증 및 권한 부여를 위해 `Filter`를 사용하는 것이 일반적입니다. 하지만 STOMP 메시지 인증에는 `Filter`를 직접 사용할 수 없습니다.

### Filter의 동작 조건

`Filter`는 Servlet 컨테이너에 의해 관리되며 **HTTP 요청**이 Servlet으로 전달되기 전/후에 동작합니다. `Filter`는 HTTP 프로토콜의 요청-응답 사이클에 묶여 있습니다.

### STOMP 메시지 발생 시

WebSocket 연결이 일단 수립되면, 그 위에서 주고받는 STOMP 메시지는 더 이상 **HTTP 요청이 아닙니다.** Servlet 요청도 아니어서 `Filter` 체인을 통과하지 않습니다. 그래서 JWT 같은 토큰 기반 인증을 `Filter`에서 처리하던 방식으로는 STOMP 메시지 인증을 수행할 수 없습니다.
...
STOMP 메시지의 인증 및 권한 부여는 Spring이 제공하는 `ChannelInterceptor`로 처리해야 합니다. `ChannelInterceptor`는 메시지 채널로 전달되는 메시지를 가로채서 처리할 수 있는 기능을 제공합니다.

### ChannelInterceptor

`ChannelInterceptor`는 클라이언트로부터 들어오는 메시지(CONNECT, SUBSCRIBE, SEND 등)가 실제 핸들러나 브로커로 전달되기 전에 가로채서 다양한 로직을 수행할 수 있습니다. 그래서 JWT 검증, 사용자 식별, 권한 확인 같은 인증·인가 처리를 여기서 할 수 있습니다.
...
`CONNECT` 메시지는 STOMP 세션이 시작될 때 단 한 번만 발생합니다. 이 시점에서 JWT를 검증하여 사용자를 인증하고 `SecurityContext`에 `Principal`을 설정해두면, 이후 `SUBSCRIBE`나 `SEND`와 같은 모든 STOMP 메시지마다 별도로 JWT를 검증할 필요가 없습니다.
...
* **연결 단위 사용자 관리**: `CONNECT` 시점에 인증된 사용자 정보를 WebSocket 세션과 연결하여 관리할 수 있습니다. 이 과정에서 특정 세션에 연결된 사용자의 정보를 쉽게 조회하고 필요에 따라 메시지를 전송하거나 권한을 확인할 수 있습니다.

> **핵심 정리**: `CONNECT` 메시지에서 JWT를 검증하는 것은 STOMP 세션당 한 번의 인증으로 이후 모든 메시지의 인증 상태를 유지하여 성능을 최적화하고 효율적인 사용자 관리를 가능하게 합니다.

## 공부하면서 느낀 점

이번 주제를 공부하면서 실시간 서비스 구현이 왜 복잡한지, 그리고 그걸 풀어온 기술들이 어떻게 발전해왔는지 이해할 수 있었습니다. 특히 HTTP의 한계를 넘어서려던 Polling과 Long Polling의 시도, 그 끝에 등장한 WebSocket, 그리고 그 위에 메시징 표준을 얹은 STOMP의 역할이 인상 깊었습니다. 기술의 기능만 아는 데서 그치지 않고 '왜 이 기술이 등장했나'를 따져보니 각 기술이 존재하는 이유와 가치가 훨씬 또렷하게 보였습니다.

Spring Security의 `Filter`가 HTTP 요청에만 걸린다는 점, 그래서 WebSocket/STOMP 환경에서는 `ChannelInterceptor`로 인증을 처리해야 한다는 점도 실무에서 흔히 부딪히는 문제에 명확한 답을 줬습니다. 특히 `CONNECT` 시점의 한 번 인증으로 세션 전체의 인증 상태를 관리하는 방식은, 성능과 보안을 함께 챙긴 효율적인 설계라는 걸 알 수 있었습니다.

여기서 얻은 내용은 앞으로 실시간 기능이 들어간 백엔드 시스템을 설계하고 구현할 때, 견고하고 확장 가능한 아키텍처를 잡는 데 큰 도움이 될 것 같습니다.
