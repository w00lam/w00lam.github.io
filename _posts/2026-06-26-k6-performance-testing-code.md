...
예를 들어 상태 코드 200 검증이 많이 실패했다면 API가 정상 응답을 유지하지 못했다는 신호다.
...
예를 들어 인기 검색어, 카테고리 목록, 인기 상품 목록처럼 반복 조회가 많은 데이터는 캐시로 DB 부하를 줄일 수 있다.
...
인증이 필요한 API라면 로그인 후 토큰을 사용한 시나리오도 작성해볼 수 있다.
...
“API가 정상 동작한다”와 “API가 트래픽을 버틴다”는 다른 문제다.
