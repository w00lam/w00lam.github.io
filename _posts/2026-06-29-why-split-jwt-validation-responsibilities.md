...
JWT를 사용하는 프로젝트를 진행하면서 인증 필터(`OncePerRequestFilter` 등)를 구현할 기회가 많았습니다. 필터 내부에서는 클라이언트가 보낸 토큰이 유효한지 검증하고, 문제가 없다면 Spring Security의 `SecurityContext`에 인증 객체를 설정해 주는 흐름을 거치게 됩니다.

처음 코드를 작성할 때는 단순히 중복을 제거하고 코드를 짧게 만드는 것에만 집중했었습니다. 하지만 검증 로직이 늘어남에 따라 "왜 이 검증 단계들을 하나로 묶지 않고 굳이 잘게 쪼개어 관리해야 하는가?"에 대한 근본적인 의문이 생겼고, 팀원들과 깊게 대화하면서 설계적 관점에서 큰 배움을 얻었습니다. 배운 내용을 TIL로 정리해 봅니다.
...
위의 코드를 보면 토큰을 파싱한 이후에 `isAccess()`, `role()`, `Blacklist`, `Invalidation` 등 4가지 이상의 검증 조건이 꼬리를 물고 나열되어 있습니다. 
...
이렇게 하면 호출하는 쪽인 `authenticate()` 메서드는 한눈에 보기에 단 몇 줄로 깔끔하게 정리될 것입니다. 하지만 다시 찬찬히 뜯어보니 이 방식은 코드 줄 수는 줄어들지 몰라도, **각 검증 단계가 지닌 구체적인 도메인 맥락과 비즈니스적 의미가 완전히 흐려지는 문제**가 있었습니다.

## 같은 claims를 사용해도 같은 책임은 아니다

가장 중요한 깨달음은 **`claims`는 단지 검증을 수행하기 위한 '재료(데이터)'일 뿐, 이를 통해 판단하려는 '책임(역할)'은 전혀 다르다**는 점이었습니다. 데이터의 모양새가 같다고 해서 그 데이터를 다루는 목적과 비즈니스 요구사항까지 하나로 묶일 수는 없었던 것입니다.
...
그러나 그 토큰을 현재 요청의 인증 수단으로 사용해도 되는지까지 판단하지는 않습니다. 즉, 기술적 검증이 끝났다고 해서 곧바로 비즈니스적으로 인증 가능한 토큰이라는 뜻은 아닙니다.
...
* **Access Token**: 짧은 수명을 가지며, 매 API 요청마다 헤더에 실려 본인을 인증하는 데 쓰입니다.
* **Refresh Token**: 상대적으로 긴 수명을 가지며, 오직 Access Token을 재발급받기 위한 엔드포인트에서만 사용됩니다.
...
`role`은 단순한 부가 메타데이터가 아니라 **인증 이후 인가 판단에 사용되는 핵심 정보**입니다. 따라서 토큰 내부에 권한 정보가 제대로 채워져 있는지를 별도로 체크하는 것은, **"이 토큰의 정보만으로 정상적인 스프링 시큐리티 인증 객체를 온전히 만들어낼 수 있는가?"**를 확인하는 객체 빌드 자격 검증 책임입니다.

---

## Blacklist는 로그아웃된 개별 토큰을 막는다

기본적으로 JWT 기반 Access Token은 서버가 상태를 갖지 않는(Stateless) 형태로 설계되지만, 실무에서는 로그아웃 등의 이유로 특정 토큰을 강제로 만료시켜야 하는 요구사항이 발생합니다. 이 문제를 해결하기 위해 도입하는 것이 Blacklist 검증입니다.
...
결국 Blacklist는 개별 토큰의 로그아웃 상태를, Token Invalidation은 사용자 계정의 보안 상태 및 일괄 만료 정책을 검증하는 서로 다른 비즈니스적 책임을 감당하고 있습니다.

---

## 무조건 합치는 것보다 의미 단위로 나누는 것이 낫다

앞서 살펴본 것처럼 모든 검증 단계는 저마다의 뚜렷한 이유와 책임을 갖고 있습니다. 만약 이를 하나의 거대한 `isValidClaims()`로 뭉뚱그려 버린다면 다음과 같은 문제점들이 생깁니다.

1. **실패 원인 파악의 모호함**: 토큰 검증이 실패했을 때 디버깅이나 모니터링 로그를 남기기 어렵습니다. 만료된 토큰인지, 블랙리스트인지, 비밀번호 변경에 의한 무효화인지 구분하기 어려워 유지보수 생산성이 떨어집니다.
2. **네트워크 I/O 비용의 결합**: 순수한 claims 내 필드 값 연산(메모리 연산)과 Redis나 RDB 조회가 필요한 정책 검증(I/O 연산)이 한곳에 뒤섞여 성능 튜닝이나 장애 격리가 곤란해집니다.
3. **수정 영향도의 전파**: 무효화 정책이나 블랙리스트 기술 스택이 변경될 때, 관련 없는 토큰 자체 검증 코드까지 함께 흔들리게 됩니다.
...
* 코드의 길이만을 줄이기 위해 맹목적으로 검증을 병합하기보다, **동일한 책임을 가지는 단위로 적절히 그룹화하고 격리하는 것**이 지속 가능한 백엔드 아키텍처를 만드는 열쇠임을 깊이 배웠습니다.

---

### 3줄 요약
1. JWT 파싱과 필드 검증, 외부 세션/스토어 조회를 통한 무효화 검증은 데이터(`claims`)를 공유할 뿐 해결하고자 하는 비즈니스 책임이 서로 다릅니다.
