...
여기서 중요한 점은 Nginx가 Spring Boot를 대신 실행하는 서버가 아니라는 것이다. Spring Boot 애플리케이션은 여전히 8080 포트에서 실행된다. 대신 Nginx가 80번이나 443번 포트에서 외부 요청을 먼저 받고, 그 요청을 내부에서 실행 중인 Spring Boot 애플리케이션으로 전달한다.

즉, 사용자는 Nginx를 바라보고, Nginx는 내부 애플리케이션을 바라보는 구조가 된다.
...
그래서 Nginx는 외부 사용자와 내부 애플리케이션 사이의 경계 역할을 한다고 이해했다. 외부에는 표준 포트인 80/443만 열어두고, Spring Boot의 8080 포트는 내부에서만 사용하도록 둘 수 있다.
...
즉, `proxy_pass`는 Reverse Proxy 구조에서 요청 전달 방향을 정하는 핵심 설정이라고 볼 수 있다.
...
처음에는 `proxy_set_header`가 부가 설정처럼 보였지만, 실제 운영 환경에서는 “원래 요청이 어떤 모습이었는지”를 애플리케이션에 전달하는 중요한 역할을 한다고 이해했다.
...
그래도 Nginx를 앞단에 두면 적어도 외부 요청 경로와 내부 실행 구조를 분리할 수 있고, 이후 Blue-Green 배포 같은 구조로 확장할 수 있는 기반이 생긴다고 이해했다.
...
결국 이번에 내가 이해한 Nginx는 다음과 같다.

> Nginx는 Spring Boot를 대신 실행하는 서버가 아니라, 외부 요청을 먼저 받고 내부 Spring Boot 애플리케이션으로 전달하는 앞단 서버다.

이 관점이 생기고 나니 `proxy_pass`, `proxy_set_header`, HTTPS 처리, Docker Compose 내부 네트워크, Blue-Green 배포가 서로 따로 떨어진 개념처럼 보이지 않았다.
