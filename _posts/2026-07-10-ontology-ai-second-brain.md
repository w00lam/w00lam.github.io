...
RAG의 검색 단계가 아직 낯설다면 먼저 [RAG의 핵심은 생성이 아니라 검색이다](/posts/rag-search-not-generation/)를 읽어 보면, 아래에서 설명할 구조를 이해하는 데 도움이 된다.
...
임베딩과 벡터 데이터베이스의 역할을 더 자세히 보고 싶다면 [Vector DB 완전 정복: 개념부터 실무 활용까지](/posts/vector-db-guide/)를 함께 참고할 수 있다.
...
Redis를 캐시 관점에서 더 살펴본 기록으로는 [Spring Cache: 캐시 Key 설계부터 동기화, RedisTemplate 동작 원리까지](/posts/spring-cache-design-sync-redistemplate/)도 연결해 볼 만하다.
