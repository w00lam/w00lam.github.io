...
UUID를 사용하면 애플리케이션 레벨에서 고유 ID를 생성할 수 있어, 여러 서버 인스턴스가 동시에 데이터를 생성하더라도 ID 충돌 가능성을 낮출 수 있다. Auto Increment 기반의 `Long` ID는 `/api/members/1`, `/api/members/2`처럼 데이터 생성 순서가 외부에 노출되어 다음과 같은 문제가 발생할 수 있다.
