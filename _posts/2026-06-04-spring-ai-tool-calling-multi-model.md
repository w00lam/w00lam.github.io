...
지난 [Spring AI로 AI 앱 개발 시작하기: ChatClient와 Structured Output](/posts/spring-ai-chatclient-structured-output/) 포스팅에서 Spring AI의 기본적인 개념과 `ChatClient`, `Structured Output`의 중요성에 대해 이론적으로 살펴보았습니다. 오늘은 그 이론을 실제 `multimodal-spring-ai-boot4` 프로젝트 안에서 코드로 확인하고, 특히 **Tool Calling 설계**와 **멀티 모델 활용 전략**에 집중하여 깊이 있는 이해를 목표로 합니다.

AI 프로젝트는 AI에게 일을 한 번에 맡기지 않습니다. 이는 마치 복잡한 비즈니스 로직을 하나의 거대한 함수로 처리하지 않고, 여러 작은 함수와 클래스로 분리하여 관리하는 것과 유사합니다. AI에게 "영수증 이미지 + 사용 목적 -> AI야, 이거 경비 처리 가능한지 한 번에 판단해줘 -> 최종 답변"과 같이 한 번에 모든 것을 맡기는 방식은 다음과 같은 문제를 야기합니다.
...
이러한 문제를 해결하기 위해 AI 기능을 단계별로 나누어 설계하는 것이 중요합니다.
...
이렇게 두 번의 AI 호출로 나누는 이유는 각 단계의 책임을 분리하여 디버깅을 용이하게 하고, 오류의 원인을 명확히 파악하기 위함입니다.
...
오늘 프로젝트에서는 `defaultTools`가 아니라 요청 안에서 `.tools(expensePolicyTools)`를 사용합니다. 이는 모든 AI 요청에 도구를 열어두는 것이 아니라, 경비 검토 요청에서만 경비 규정 도구를 제공하여 권한을 최소화하는 좋은 예시입니다.
...
서비스 코드는 특정 provider 이름보다 `geminiChatClient`, `anthropicChatClient`와 같이 역할에 맞는 이름으로 `ChatClient`를 주입받아 사용하는 것이 좋습니다. 이는 코드의 가독성과 유지보수성을 높여줍니다.

## 5. 오늘의 정리!

이번 Spring AI 실습을 통해 AI 백엔드 개발에서 중요한 것은 단순히 모델이 똑똑하다는 사실이 아니라는 것을 다시 한번 깨달았습니다. 핵심은 **모델을 우리 서비스의 책임, 데이터 계약, 보안 경계, 관찰 가능성 안에 넣을 수 있느냐**입니다.
