...
지난 [Spring AI로 AI 앱 개발 시작하기: ChatClient와 Structured Output](/posts/spring-ai-chatclient-structured-output/) 포스팅에서 Spring AI의 기본적인 개념과 `ChatClient`, `Structured Output`의 중요성을 이론적으로 살펴보았습니다. 오늘은 그 이론을 실제 `multimodal-spring-ai-boot4` 프로젝트 안에서 코드로 확인해 보겠습니다. 특히 **Tool Calling 설계**와 **멀티 모델 활용 전략**을 깊이 있게 들여다보겠습니다.
...
| **회사 규정은 Tool로 조회한다** | 모델이 내부 규정을 추측하지 않게 하고 실제 데이터로 판단하게 함 |
...
Mock 모드에서는 Tool Calling이 실제로 일어나지 않습니다. `check-expense` API를 호출해도 모델이 도구를 선택하고 호출하는 실제 과정은 볼 수 없습니다. 실제 Tool Calling의 동작을 확인하려면 OpenAI 모델 모드로 실행해야 합니다.
...
1. **첫 번째 AI 호출**: 영수증 이미지와 추출 프롬프트로 `ReceiptExtraction` DTO를 생성합니다. (보이는 값을 읽는 일)
...
* **중간 결과는 DTO로**: `Structured Output`로 LLM의 응답을 구조화된 데이터로 받아 후속 로직의 안정성과 검증 가능성을 높인다.
* **외부 지식은 Tool로 조회**: LLM이 모르는 도메인 특화 지식이나 실시간 정보는 `Tool Calling`로 외부 시스템에서 안전하게 조회하도록 설계한다.
